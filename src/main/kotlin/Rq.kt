...
// Request 라고 하지 않는 이유는, 이미 선점되어 있는 클래스명 이기 때문이다.
class Rq(command: String) {
    // 데이터 예시
    // 전제 URL : /article/detail?id=1
    // articlePath : /article/detail
    val actionPath: String

    // 데이터 예시
    // 전체 URL : /article/detail?id=1&title=안녕
    // paramMap : {id:"id", title:"안녕"}
    private val paramMap: Map<String, String>

    // 객체 생성시 들어온 command 를 ? 기준으로 나눈 후 추가 연산을 통해 actionPath와 paramMap의 초기화한다.
